...
class PriceVariationLimiterOrderValidatorTest {
	...
	for (int i = 0; i < rawOptionsTestData.length; i++) {
	    ...
	    OrderValidateResult result = PriceVariationLimiterOrderValidator.validatePriceVariation(order, strategy,
		    new BigDecimal(data[6]), tickTable);
	    logger.info(i + "," + result.message());
	    assertEquals(new BigDecimal(data[8]).setScale(1),
		    new BigDecimal(extractField(result.message(), "ActualVariation")).multiply(BigDecimal.valueOf(-1))
			    .setScale(1),
	...
	for (int i = 0; i < rawStocksTestData.length; i++) {
	    ...
	    Order order = createOrder(data[1], data[3], data[4], Double.parseDouble(data[5]));
	    OrderValidateResult result = PriceVariationLimiterOrderValidator.validatePriceVariation(order, strategy,
		    new BigDecimal(data[6]), null);
	    logger.info(i + "," + result.message());
	    assertEquals(Double.parseDouble(data[8]),
	...
	Order order = OrderUtils.createOrder(InstrumentType.valueOf(instrumentType), symbol, TradeSide.valueOf(side),
		price, 1);
	return order;
    }
    ...
    String[] rawOptionsTestData = {
	    // strategy, type, variation, symbol, side, price, reference price, expected
	    ...
	    "TickSize, Option, 8.0,  KS200400F5.KS, Buy, 10.60, 10.15, Pass, -9", // original data is 10.06, should be
										  // 10.60
    ...
    String[] rawStocksTestData = { "Absolute, Stock, 10.0, VOD.L, Buy, 245.0, 245.0, Pass, 0",
